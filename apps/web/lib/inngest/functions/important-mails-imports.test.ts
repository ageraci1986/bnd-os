...
/**
 * Pinned guard (Plan 3b Task 6 — same rationale as
 * `morning-briefing-imports.test.ts` / `blocked-cards-scan-imports.test.ts`):
 * the mails-importants Inngest function never runs an agent turn (it's the
 * deterministic §6.3-adjacent heuristic scan + notice creation over
 * `EmailMessage`/`Contact`), so it must never import the agent provider or
 * tool registry either. See `morning-briefing-imports.test.ts`'s header
 * comment for why static source inspection (not `vi.mock`) is the right
 * method here.
 */
...
describe('Inngest functions — no provider/registry import (pinned)', () => {
  it('important-mails.ts imports neither @nexushub/agent, provider, nor registry', () => {
    assertNoForbiddenImports('important-mails.ts');
  });
});
