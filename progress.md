...
| 2026-07-27 | Assistant | Plan 1/4  | **Assistant NexusHub — Cœur agent** (spec `docs/superpowers/specs/2026-07-27-assistant-agent-design.md`, plan `docs/superpowers/plans/2026-07-27-assistant-core.md`, branche `worktree-assistant-agent`). Nouveau package `@nexushub/agent` (pur TS, coverage 100 % : ToolRegistry + `defineTool`, boucle `runTurn` avec gate par action, garde admin, abort propagé, protection tool_use tronqués). Seam provider `@anthropic-ai/sdk@0.115.0` (seul fichier qui importe le SDK). 7 tools de lecture workspace-scoped (gate ownership sur `read_mail`, jour UTC via `startOfTodayUtc`, sorties bornées). Route SSE `/api/assistant/chat` (auth + CSRF + rate limit 30/5min + plafonds 150k/40 msgs + AbortSignal). Page `/assistant` avec chat streaming (historique éphémère plafonné client, réponses partielles préservées, a11y AA). Exécution subagent-driven : 11 tâches × (impl + revue spec + revue qualité), 1 critique corrigé (fuite corps de mails inter-boîtes). Gates : typecheck/lint 6/6, ~500 tests verts. Reste : vérif manuelle E2E (attente `ANTHROPIC_API_KEY`), puis Plans 2-4 (tools CRUD + gate UI, proactivité + mémoire, orbe animée + E2E).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 2026-07-27 | Assistant | Plan 2a/4 | **Assistant — Actions Kanban + gate de confirmation** (plan `docs/superpowers/plans/2026-07-27-assistant-actions.md`, branche `feat/assistant-actions`). ConfirmStore Redis (nonce single-use, timeout 120 s fail-closed, abort-aware, hard-fail prod sans Upstash) + endpoint `POST /api/assistant/confirm` (rate-limité 20/min). Fonctions core extraites iso-comportement (`createCardCore`/`deleteCardCore`/`createProjectCore`) réutilisées par les actions form ET les tools. 8 tools mutants Kanban via `defineTool` (`delete_card` gated ; `safeMutation` anti-fuite d'erreurs internes ; garde format date YYYY-MM-DD ; enums templates générés depuis `@nexushub/domain`). Route chat : confirmer temps réel (SSE `confirm_request`/`confirm_resolved`), audit `assistant_turn`/`assistant_gate`/`assistant_tool_run` (noms de tools uniquement, jamais d'arguments), `maxDuration 300`. Dialog Allow/Deny inline (verrou de réponse, focus management WCAG AA, sticky-scroll, nettoyage au stream mort). Migration `assistant_audit_actions` **appliquée sur bnd-os-staging**. Audit `card_deleted` (trou comblé). Gates : 924 tests verts 5 packages, typecheck/lint 6/6. Reste : Plan 2b (mail lazy-body + brouillons + envoi gated + widgets structurés), Plan 3 (proactivité + mémoire), Plan 4 (orbe + E2E).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 2026-07-28 | Assistant | Plan 2b/4 | **Assistant — Mail + widgets structurés** (plan `docs/superpowers/plans/2026-07-27-assistant-mail-widgets.md`, branche `feat/assistant-mail-widgets`). Seam agent étendu (`describeForConfirm` avec contrat PII, `Confirmer(description, tool)`, `tool_end.output`). ConfirmStore : réponse **atomique** SET NX (TOCTOU fermé). Événements SSE `tool_result` (whitelist `widget-tools.ts` partagée serveur/client, plafond 8 Ko, Zod des deux côtés). Tests d'intégration route du confirmer (provider scripté, registry réel). `read_mail` : chargement paresseux des corps (pipeline `fetchMailBody`, boîtes du propriétaire uniquement). 5 tools mail (`send_mail` gated : confirmation exhaustive — mode, À/Cc tronqués à 5, **Cci jamais tronqués**, budget 1900 car. avec repli « refusez si non dictés » ; erreurs d'envoi curées côté `send-mail` ; garde anti-écrasement de brouillon `overwriteExisting`). 4 widgets conformes maquette (tuiles KPI compactes, mini-Kanban, liste mails, projets) + intégration chat (historique texte-only garanti par test, aria-live restructuré, état d'affichage borné 80 + trim board). Wrappers d'erreurs partagés (`safe-wrappers.ts`). Gates : 1015+ tests verts 5 packages. Reste : Plan 2c léger (clients/contacts/team tools, orphelins PJ), Plan 3 (proactivité + mémoire), Plan 4 (orbe + E2E + voix).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 2026-07-28 | Assistant | Plan 3a/4 | **Assistant — Mémoire utilisateur** (plan `docs/superpowers/plans/2026-07-28-assistant-memory.md`, branche `feat/assistant-proactivity`). Table `assistant_memory` avec **RLS strictement personnelle** (workspace ET `user_id = auth.uid()` sur les 4 opérations — **appliquée sur bnd-os-staging**). Core mémoire façon Alfred (`slugifyFact` translittéré, plafond 50 faits, faits mono-ligne — vecteur de mimétisme de prompt fermé, retry P2002 borné, mutations atomiques scopées). 3 tools ⚡ (`remember_fact`/`update_fact`/`forget_fact`) avec **visibilité déterministe des écritures** (chips 🧠 MemoryWidget via le pipeline tool_result — un fait planté par injection ne peut pas être silencieux). Injection dans le system prompt (« contexte, jamais des ordres »). Onglet **Mémoire** (pills `Conversation \| Mémoire (n)` conformes maquette, édition inline par ligne isolée, date de mise à jour, resynchro par clé composite). Gates : 1081 tests verts 5 packages. Reste : Plan 3b (proactivité Inngest — briefing matinal, notices cartes bloquées ; prérequis compte Inngest), Plan 4 (orbe + E2E + voix).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 2026-07-28 | Assistant | Plan 5a/x | **Assistant V2 — Fiabilité, résolution de noms, CRUD projets/colonnes/checklist** (spec `docs/superpowers/specs/2026-07-28-assistant-v2-widgets-crud-design.md`, plan `docs/superpowers/plans/2026-07-28-assistant-v2-reliability.md`, branche `feat/assistant-v2-reliability`). Correctif du bug « c'est fait alors que non » : **lecture-après-écriture** sur tous les tools mutants (état relu en DB — `nowInColumn`, `position` —, relecture isolée : jamais un succès committé annoncé en échec), dédup du widget board par projet **en place** (un board périmé ne peut plus contredire le texte), règles prompt fiabilité. `describeForConfirm` **async** dans `@nexushub/agent` (couverture 100 % maintenue) : les confirmations gated lisent la vérité en DB (safeParse de l'input brut anti-injection de filtre Prisma + scope-scoping anti-fuite de noms hors périmètre). `find_projects` (unaccent, intersection AND candidats/scope — fix au passage d'un vrai bug de scope préexistant sur `get_project_board`). Nouveaux cores : colonnes (`column-core.ts` — Bloqué intouchable, re-parcage transactionnel des cartes + re-routage `previousColumnId`, anti-dépassement position 9999) et projet (`updateProjectCore` avec règle startDate ≤ endDate + P2002, `deleteProjectCore` iso-extrait). 8 nouveaux tools (update/delete_project ⚡, add/rename/reorder/delete_column ⚡, get_card_details borné, set_checklist_item ancré sur la carte réelle de l'item + auto-avancement immédiat). Widget `find_projects` (allowlist + ProjectListWidget). Gates : 17/17 tâches turbo, 816 tests web + 42 agent, typecheck/lint verts. Reste : Plans 5b (CRUD clients/équipe/templates + mails en masse) et 5c (widgets mail interactifs + brouillon éditable + deep-link).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-07-28 | Assistant | Plan 5b/x | **Assistant V2 — CRUD clients/contacts, équipe, templates, mails en masse** (plan `docs/superpowers/plans/2026-07-28-assistant-v2-crud-mail-bulk.md`, branche `feat/assistant-v2-crud-bulk`). Migration `assistant_crud_audit_mail_archive` (12 valeurs AuditAction + `EmailMessage.archivedAt` — **appliquée sur bnd-os-staging**). Cores clients/contacts extraits iso-comportement (6 cores + audits, messages ADR #14 exacts, email normalisé) et cores équipe (protection dernier Admin surfacée via helper partagé, rate limiter d'invitation partagé avec l'UI, token jamais exposé). **14 nouveaux tools** : 7 clients/contacts (deletes gated, describe delete*client déclaratif si projets actifs, cap domains post-normalisation, RACI source unique `set_contact_raci` — global au contact, modèle réel), 3 équipe **tous adminOnly ET gated** (décision revue : la promotion Admin = escalade max par injection ; describes avec « ADMINISTRATEUR accès complet », cas « votre propre rôle », email normalisé affiché), 3 templates (delete gated, builtin déclaratif, audits template*\* ajoutés aux cores), 4 mail-state bulk owner-only (`mark_mail_read/unread`, `archive_mail` ⚡, `delete_mail` ⚡ — plafond 100, skipped compté, wording honnête « action locale, peut réapparaître après sync », descriptions croisées avec le mono-mail existant). Gates : 17/17 turbo, 973+ tests web, typecheck/lint verts. Dette notée : résurrection par sync IMAP/Graph des deletes locaux (bug préexistant du sync, à traiter avant de durcir la promesse produit). Reste : Plan 5c (widgets mail interactifs + brouillon éditable + deep-link).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 2026-07-28 | Assistant | Plan 5c/x | **Assistant V2 — Widgets mail interactifs, brouillon éditable, deep-link** (plan `docs/superpowers/plans/2026-07-28-assistant-v2-mail-widgets.md`, branche `feat/assistant-v2-mail-widgets`). Canal d'interactivité des widgets (`WidgetActions.sendMessage` — messages injectés = **ids + verbes fixes uniquement**, jamais de contenu de mail : anti-promotion d'injection ; identité stable par ref). **MailListWidget = client mail embarqué** : corps dépliables (`fetchMailBody` owner-gated, HTML sanitizé uniquement, cache local), toggle lu/non-lu optimiste (`markEmailUnread` mono-mail créé, owner-only), Répondre/Transférer/Archiver/Supprimer via l'agent, « Tout marquer lu », deep-link. **Deep-link Communications `?mailbox=&mail=`** avec résolution de page serveur, sélection + markEmailRead à l'arrivée, pagination qui consomme le param. **Flux brouillon source-de-vérité-DB** : `get_draft` (texte strippé borné, jamais bodyHtml au modèle), `send_draft` ⚡ avec **jeton de fraîcheur `expectedUpdatedAt`** vérifié au describe (déclaratif si mismatch) ET au handler (fenêtre confirmation→envoi fermée côté serveur), énumération Cci exhaustive factorisée de send_mail. **MailDraftWidget = éditeur live du brouillon DB** (seed `loadDraft`, conversion HTML→texte fidèle par DOMParser — un Critical de corruption de corps attrapé et corrigé en revue —, autosave 2 s sérialisé avec flush honnête + retry, composeAttachments préservés, échappement strict sans dangerouslySetInnerHTML). Gates : 17/17 turbo, 1113+ tests web, typecheck/lint verts. Dette : sanitize mail autorise class/id (overlay Tailwind théorique, partagé MailReader — follow-up sécurité dédié) ; widgets draft d'anciens tours restent éditables sur état périmé (le jeton bloque tout envoi erroné ; follow-up : lecture seule hors dernier widget) ; pas de refresh live de l'inbox depuis le chat ; blocs riches hors p/div aplatis si corps édité ; éditions perdues dans la fenêtre de debounce au démontage ; filtres client/mailbox préservent ?mail (ré-ancrage one-shot) ; erreur de dépli non retryable ; read_mail texte-only ; replyToExternalId non passé ; attachments non éditables dans le widget ; get_today_overview compte les archivés ; a11y RecipientField (préexistant). Itération V2 complète — reste Plans 3b (Inngest) et 4 (orbe, E2E, voix). |
| 2026-07-28 | Assistant | Plan 4/4  | **Assistant — Orbe, accueil briefing, E2E, Storybook** (plan `docs/superpowers/plans/2026-07-28-assistant-orb-e2e-storybook.md`, branche `feat/assistant-orb-e2e`). **Orbe blob+halo** portée octet pour octet de la maquette validée dans le design system (`nx-orb-*`, états idle/thinking/responding pilotés par le tour, `prefers-reduced-motion` → fondus — premier usage dans le repo). **Accueil digéré** : `loadTodayOverview` extrait iso du tool, page serveur → phrase accordée (CLDR fr) + tuiles KPI hors aria-live, zéro token, fallback statique loggé sans PII. **E2E exécutables** : provider Anthropic scripté (`ASSISTANT_E2E_MOCK`, garde double anti-prod testée), sign-in helper, `/assistant` dans l'auth-gate, `assistant.spec.ts` (4 parcours spec §8 : briefing, Allow avec effet DB réel, Deny, timeout — gated env, sélecteurs vérifiés composants). **Storybook 10.5.5 réellement installé** (apps/web, nextjs-vite vérifié Context7 ; conflit vite résolu par pin ^5.4 ; mocks Storybook-only des server actions par chemin résolu ; 10 stories vérifiées Chromium headless : orbe 4 états, KPI, mail-list, mail-draft, board). Gates : 17/17 turbo, 1157 tests web, couverture agent 100 %. Dette : notices d'accueil (Plan 3b), Chromatic non câblé, E2E gated local, état d'orbe pendant le dialog de confirmation (thinking — raffinement futur), i18n du chat (préexistant). Itération assistant V1 complète — reste Plan 3b (Inngest, prérequis clés).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 2026-07-28 | Assistant | Plan 3b/4 | **Assistant — Proactivité Inngest** (plan `docs/superpowers/plans/2026-07-28-assistant-proactivity-inngest.md`, branche `feat/assistant-proactivity-inngest`). Migration `assistant_proactivity` (**appliquée sur bnd-os-staging**) : kinds `agent_briefing/agent_card_blocked/agent_mail_important`, canal `in_app`, kill switch + opt-in briefing sur Membership. **Inngest 4.13.0 réellement installé** (Context7 ; la spec §10 le croyait présent — corrigée) : client + route `/api/inngest` + 3 crons (pattern pure-core testable + garde d'imports pinnée : AUCUN provider/registry dans les crons — zéro token). **Briefing matinal** 07:30 Europe/Brussels jours ouvrés (opt-in, phrase digérée partagée avec l'accueil). **Scan horaire des échéances** : le job aspirationnel de CLAUDE.md §6.3 devient réel — ET les notices partent aussi du chemin reconcile-on-read (fix de revue : sinon le cron était quasi muet en workspace actif) avec steps Inngest retry-safe (scan mémoïsé / notify séparé). **Mails importants** (contact client connu, non lu > 4 h, owner-only, borné 500, dédup). `createAgentNotice` unique (kill switch → opt-in → préférence → dédup ordonnée) ; première écriture de `readAt` du repo (`markNotificationRead`). **UI** : pile de notices sur /assistant (« En discuter » injecte le `discuss` DB — ids+verbes, RLS INSERT bloquée aux authenticated : chemin d'injection fermé —, « Ignorer » optimiste), point animé sidebar (reduced-motion ok), section **Settings réelle** (kill switch + toggles par type, sauvegarde auto + toast — ADR #10). Gates : 17/17 turbo, 1290 tests web, couverture agent 100 %. Dette : perte de notice si notify échoue durablement sur le read-path (best-effort loggé), bornes longueur mapper, Communications non scopé (préexistant), web push/cloche/quiet hours/TZ par user.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

| 2026-07-28 | — | cards | **Template de carte : visibilité + défaut garanti.** Deep-link `/templates/cards?template=<id>`, lien « Modifier le template » dans le rail latéral du modal carte, bootstrap d'un template « Standard » (description + checklist) par défaut à la création de workspace, migration `20260728120000` appliquée sur staging (promotion/insertion du défaut + backfill des cartes sans template — 0 workspace sans défaut, 0 carte active sans template). Spec `docs/superpowers/specs/2026-07-28-card-template-visibility-design.md`. |

| 2026-08-03 | Assistant | Voix V1.5 | **Assistant — Mode voix (V1.5), livré.** Push-to-talk ⌥ Option (+ bouton micro), STT **Deepgram nova-3**, TTS **ElevenLabs flash** en streaming phrase par phrase, sortie audio symétrique avec interruption (nouvelle prise de parole coupe la lecture en cours), confirmation vocale sur motifs stricts (pas d'inférence floue « oui/non »), capsule d'état (`voice-capsule.tsx` — idle/recording/transcribing/speaking/denied) + orbe en état `listening`, routes voice dédiées rate-limitées, E2E gated avec media factice (fake-media Playwright). Polish revue : test `deriveOrbActivity` renommé pour dire l'invariant réel (« sans flag listening, ne dérive jamais listening ») ; wrapper `aria-live` de la capsule **toujours monté** (même en idle, vide) pour une annonce fiable idle→recording par les lecteurs d'écran (VoiceOver notamment). **Reste à faire** : renseigner `DEEPGRAM_API_KEY`, `ELEVENLABS_API_KEY`, `ELEVENLABS_VOICE_ID` dans Vercel + `.env.local` (à demander à Angelo — routes → 503 tant qu'absentes) ; passe E2E voix complète en local (identifiants E2E requis) ; **dette i18n assistant** (préexistante, étendue par la voix) — toute la page `/assistant` est en FR inline, la spec voix promettait next-intl, à traiter en un seul lot dédié ; vérification manuelle lecteur d'écran (VoiceOver/NVDA) de la capsule voix ; observabilité Sentry des routes voice (503/502 silencieux — à décider pour les deux routes ensemble). |
| 2026-08-03 | Assistant | Visibilité totale | **Assistant — Visibilité totale (notifications + exhaustivité), livré.** Spec `docs/superpowers/specs/2026-08-03-assistant-total-visibility-design.md`. Origine : retours de test d'Angelo — « notification: 1 » inexplicable dans le briefing, « marque tous les mails de X comme lus » impossible au-delà de la page de recherche. **Tools notifications** (`list_notifications`/`mark_notifications_read`, non gated, strictement `userId`, réutilise le mapping label FR existant des notices — pas de `data` JSON brut exposé). **3 tools bulk mail par filtre serveur** (`mark_mails_read_by_filter`/`archive_mails_by_filter`/`delete_mails_by_filter`, gated ⚡, filtre Zod à au moins un critère, owner-only, `describeForConfirm` annonce le **compte réel** via `count()` avec le même `where` que l'exécution, sans plafond). **Pagination `search_mails`** (`offset`, sortie `{ total, offset, mails }`, page max 25 conservée). **Indicateurs de troncature honnêtes** sur les autres listes plafonnées (`total`/`truncated` sur `list_projects`, `get_team_members`, checklists, colonnes de board — pas d'offset ajouté, YAGNI pour une agence 5-20 personnes). **Règles d'exhaustivité** ajoutées au system prompt (boucler sur `offset` avant de conclure, utiliser les `*_by_filter` pour « tous les… » en mutation mail, toujours annoncer une troncature). Déviations validées en revue (spec amendée en conséquence) : audit des bulk-by-filter porté par les enums existants (`mail_marked_read`/`mail_archived`/`mail_deleted`) avec `subjectType: 'mail_bulk_filter'` plutôt qu'une nouvelle action d'enum (évite une migration) ; `fromContains` insensible à la casse (Prisma `insensitive`) mais pas aux accents (unaccent SQL brut hors scope, noté dette) ; `folder` en enum strict `['inbox','sent']` (valeurs réellement écrites en base, pas de `'draft'`). Gates : typecheck/lint verts, ~1443 tests web + 94 agent. **Reste** : tests manuels Angelo (« explique ma notification », « marque tous les mails de X comme lus » avec compte annoncé vérifié, lecture au-delà de 25 mails via pagination). |
