...
> Guide opérationnel pour tout assistant (Claude Code, IA, dev humain) travaillant sur le projet **NexusHub**.
...
- **Storybook** 10 (`apps/web/.storybook`, framework nextjs-vite — composants UI isolés ; Chromatic à câbler)
...
│   ├── agent/                    # Boucle agent pure TS (assistant) — registry, gate, runTurn ; coverage 100%
│   ├── integrations/             # Adapters Slack / Graph (factices testables)
│   └── ui/                       # Design system (tokens + composants ; stories dans apps/web)
├── docs/
...
8. **Mettre à jour ce CLAUDE.md** dès qu'une décision technique est prise (nouvelle dep, nouveau pattern, nouveau risque).
